...
#'   \item **Executes Bootstrap**: Runs `B` iterations using `future_lapply`. Inside each iteration, the `model_func` is called on the full dataset (resampling logic is typically handled within `model_func`, e.g., `ridge_permutation_scorer`).
...
#'   \item **执行 Bootstrap**：使用 `future_lapply` 运行 `B` 次迭代。在每次迭代中，调用 `model_func`（重抽样逻辑通常在 `model_func` 内部处理，例如 `ridge_permutation_scorer`）。
...
#'   Defaults to `ridge_permutation_scorer`.
#'   针对每个 Bootstrap 样本执行的内部模型函数。默认为 `ridge_permutation_scorer`。
...
#' @export
run_oob_permutation = function(data, 
                         mix_name,
                         dependent_var = "y",
    ...
    # [核心修复]: 提取祖传尺子。如果没有传（比如独立测试时），就报错或回退（这里强烈建议报错，保持强一致性）
    if(is.null(args$model_knots) || is.null(args$model_boundary)){
        stop("Critical Error: 'model_knots' and 'model_boundary' must be passed to run_oob_permutation to maintain global scale.")
    }
    ...
    # [核心修复]: 仅执行一次非线性基函数展开，必须带上绝对尺子！
    full_spline_data = expand_func(data, mix_name, df_spline = df_val, 
